...
class TestPaymentService:
    ...
    def test_fee_calculation_basic(self):
        ...
        assert fees['percentage_fee'] == Decimal('7.99')
        assert fees['total_fee'] == Decimal('8.98')
        assert fees['net_amount'] == Decimal('91.02')
        assert round(fees['fee_percentage'], 2) == Decimal('8.98')

    def test_fee_calculation_small_amount(self):
        """Testa cálculo com valores pequenos"""
        fees = PaymentService.calculate_fees(10)
        assert fees['fixed_fee'] == Decimal('0.99')
        assert fees['percentage_fee'] == Decimal('0.80')
        assert fees['total_fee'] == Decimal('1.79')
        assert fees['net_amount'] == Decimal('8.21')

    def test_fee_calculation_large_amount(self):
        """Testa cálculo com valores grandes"""
        fees = PaymentService.calculate_fees(1000)
        assert fees['fixed_fee'] == Decimal('0.99')
        assert fees['percentage_fee'] == Decimal('79.90')
        assert fees['total_fee'] == Decimal('80.89')
        assert fees['net_amount'] == Decimal('919.11')
    ...
    def test_calculate_creator_earnings(self):
        ...
        assert round(Decimal(str(earnings['total_net'])), 2) == Decimal('319.06')
...
class TestTransactionModel:
    ...
    def test_transaction_auto_calculation(self, app):
        ...
        with app.app_context():
            ...
            assert transaction.percentage_fee == Decimal('7.99')
            assert transaction.total_fee == Decimal('8.98')
            assert transaction.net_amount == Decimal('91.02')
    ...
    def test_transaction_recalculation(self, app):
        ...
        with app.app_context():
            ...
            assert transaction.net_amount == Decimal('45.01')

            transaction.amount = 200
            transaction.calculate_fees()
            assert transaction.fixed_fee == Decimal('0.99')
            assert transaction.percentage_fee == Decimal('15.98')
            assert transaction.total_fee == Decimal('16.97')
            assert transaction.net_amount == Decimal('183.03')
